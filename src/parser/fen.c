...
/* Basic lookup table for piece types.
 */
static constexpr
 ...
 *
 * TODO:
 *   - add faster SIMD FEN parsing (this could be useful for fast parsing PGNs
 *   - add checked/unchecked variants, possibly a global enum with parsing flags for all parsers
 *   - add support for chess 960 FENs using "Shredder FENs"
 */
Board parse_fen(const char *fen_string, bool *white_to_move, bool *ok)
{
	...
	while (sq != 8 || file != 8)
	{
		...
		constexpr char lower_mask = 0x20;
	...
	if (*white_to_move)
		...
		board.white = byteswap(black | en_passant_mask);
	}
